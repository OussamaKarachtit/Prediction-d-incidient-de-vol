...
kafka_config = {
    ...
    'group.id': 'population_consumer',
    'auto.offset.reset': 'earliest'
}

topic = "population_topic"

bigquery_client = bigquery.Client()
table_id = "primeval-truth-445016-d4.incident_data.POPULATION_DATA"
...
def clean_data(row):
    return {
        ...
        "densite_population": row.get("densite_population"),
        "taux_chomage": row.get("taux_chomage"),
        "niveau_vie": row.get("niveau_vie", "Inconnu"),
...
print("Consuming population data from Kafka...")
try:
    while True:
